...
<#
.Synopsis
Test the Assert-NotFalse cmdlet.
.Description
Test the Assert-NotFalse cmdlet.
.Inputs
None
...
.\Test-Assert-NotFalse.ps1 -logger $simpleLogger
...
.\Test-Assert-NotFalse.ps1 -logger $customLogger
...
& {
    $test = newTestLogEntry 'Test Assert-NotFalse with get-help -full'

    $test.Data.in  = @{name = 'Assert-NotFalse'}
    ...
    $test.Pass =
        ...
        ($test.Data.out[0].Name.Equals('Assert-NotFalse', [System.StringComparison]::OrdinalIgnoreCase)) -and
...
& {
    $test = newTestLogEntry 'Test Assert-NotFalse parameters'
    ...
    if (Test-NotNull $test.Data.err) {
        $message = 'Assert-NotFalse failed parameter tests (could not get command).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }

    $paramSets = @($test.Data.out[0].ParameterSets)
    if (1 -ne $paramSets.Count) {
        $message = 'Assert-NotFalse failed parameter tests (more than one parameter set).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
    ...
    if (Test-Null $valueParam) {
        $message = 'Assert-NotFalse failed parameter tests (no parameter with name "Value").'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
    ...
    if (-not $test.Pass) {
        $message = 'Assert-NotFalse failed parameter tests ("Value" parameter attributes).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}

& {
    $test = newTestLogEntry 'Test Assert-NotFalse with Boolean $true'

    $test.Data.in  = @{value = $true}
    $test.Data.err = try {Assert-NotFalse $test.Data.in.value -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Test Assert-NotFalse with Boolean $false'
    ...
    $test.Pass =
        ...
        ($test.Data.err.FullyQualifiedErrorId.Equals('AssertionFailed,Assert-NotFalse', [System.StringComparison]::OrdinalIgnoreCase)) -and
        ($test.Data.out.Count -eq 0)
...
& {
    $test = newTestLogEntry 'Test Assert-NotFalse with $null'
...
& {
    for ($i = 0; $i -lt $nonBooleanTrue.Count; $i++) {
        $test = newTestLogEntry 'Test Assert-NotFalse with Non-Boolean that is convertible to $true'
        ...
        if (-not $test.Pass) {
            $message = 'Assert-NotFalse failed with Non-Boolean that is convertible to $true.'
            throw New-Object 'System.Exception' -ArgumentList @($message)
        }
    }
    ...
    if ($i -eq 0) {
        ...
        $message = 'No data to test Assert-NotFalse with Non-Boolean that is convertible to $true.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
...
& {
    for ($i = 0; $i -lt $nonBooleanFalse.Count; $i++) {
        $test = newTestLogEntry 'Test Assert-NotFalse with Non-Boolean that is convertible to $false'
        ...
        if (-not $test.Pass) {
            $message = 'Assert-NotFalse failed with Non-Boolean that is convertible to $false.'
            throw New-Object 'System.Exception' -ArgumentList @($message)
        }
    }
    ...
    if ($i -eq 0) {
        ...
        $message = 'No data to test Assert-NotFalse with Non-Boolean that is convertible to $false.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
