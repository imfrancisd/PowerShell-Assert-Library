...
.\Test-Assert-Null.ps1 -logger $simpleLogger
...
.\Test-Assert-Null.ps1 -logger $customLogger
...
& {
    $test = newTestLogEntry 'Test Assert-Null with get-help -full'

    $test.Data.in  = @{name = 'Assert-Null'}
    ...
    $test.Pass =
        ...
        ($test.Data.out[0].Name.Equals('Assert-Null', [System.StringComparison]::OrdinalIgnoreCase)) -and
...
& {
    $test = newTestLogEntry 'Test Assert-Null parameters'
    ...
    if (Test-NotNull $test.Data.err) {
        $message = 'Assert-Null failed parameter tests (could not get command).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }

    $paramSets = @($test.Data.out[0].ParameterSets)
    if (1 -ne $paramSets.Count) {
        $message = 'Assert-Null failed parameter tests (more than one parameter set).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
    ...
    if (Test-Null $valueParam) {
        $message = 'Assert-Null failed parameter tests (no parameter with name "Value").'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
    ...
    if (-not $test.Pass) {
        $message = 'Assert-Null failed parameter tests ("Value" parameter attributes).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}

& {
    $test = newTestLogEntry 'Test Assert-Null with Boolean $true'

    $test.Data.in  = @{value = $true}
    $test.Data.err = try {Assert-Null $test.Data.in.value -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out

    $test.Pass =
        ($test.Data.err -is [System.Management.Automation.ErrorRecord]) -and
        ($test.Data.err.FullyQualifiedErrorId.Equals('AssertionFailed,Assert-Null', [System.StringComparison]::OrdinalIgnoreCase)) -and
...
& {
    $test = newTestLogEntry 'Test Assert-Null with Boolean $false'
...
& {
    $test = newTestLogEntry 'Test Assert-Null with $null'
...
& {
    for ($i = 0; $i -lt $nonBooleanTrue.Count; $i++) {
        $test = newTestLogEntry 'Test Assert-Null with Non-Boolean that is convertible to $true'
        ...
        if (-not $test.Pass) {
            $message = 'Assert-Null failed with Non-Boolean that is convertible to $true.'
            throw New-Object 'System.Exception' -ArgumentList @($message)
        }
    }
    ...
    if ($i -eq 0) {
        ...
        $message = 'No data to test Assert-Null with Non-Boolean that is convertible to $true.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
...
& {
    for ($i = 0; $i -lt $nonBooleanFalse.Count; $i++) {
        $test = newTestLogEntry 'Test Assert-Null with Non-Boolean that is convertible to $false'
        ...
        if (-not $test.Pass) {
            $message = 'Assert-Null failed with Non-Boolean that is convertible to $false.'
            throw New-Object 'System.Exception' -ArgumentList @($message)
        }
    }
    ...
    if ($i -eq 0) {
        ...
        $message = 'No data to test Assert-Null with Non-Boolean that is convertible to $false.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
