...
.\test-test-notnull.ps1 -logger $simpleLogger
...
.\test-test-notnull.ps1 -logger $customLogger
...
& {
    ...
    $test.Data.err = try {$test.Data.fullHelp = Get-Help Test-NotNull -Full} catch {$_}

    $test.Pass =
        ($null -eq $test.Data.err) -and
        ($test.Data.fullHelp.Name -is [System.String]) -and
        ($test.Data.fullHelp.Name.Equals('Test-NotNull', [System.StringComparison]::OrdinalIgnoreCase)) -and
...
& {
    ...
    $test.Data.paramSets = @((Get-Command -Name Test-NotNull).ParameterSets)
...
& {
    ...
    $test.Data.err = try {Test-NotNull $true -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    ...
    $test.Data.err = try {Test-NotNull $false -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    ...
    $test.Data.err = try {Test-NotNull $null -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Test Test-NotNull with Non-Booleans that are convertible to $true'
    ...
    for ($i = 0; $i -lt $nonBooleanTrue.Count; $i++) {
        $test.Data.err[$i] = try {Test-NotNull $nonBooleanTrue[$i] -OutVariable out | Out-Null} catch {$_}
        $test.Data.out[$i] = $out
...
& {
    $test = newTestLogEntry 'Test Test-NotNull with Non-Booleans that are convertible to $false'
    ...
    for ($i = 0; $i -lt $nonBooleanFalse.Count; $i++) {
        $test.Data.err[$i] = try {Test-NotNull $nonBooleanFalse[$i] -OutVariable out | Out-Null} catch {$_}
        $test.Data.out[$i] = $out
