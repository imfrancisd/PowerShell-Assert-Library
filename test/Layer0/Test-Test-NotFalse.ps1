...
.\test-test-notfalse.ps1 -logger $simpleLogger
...
.\test-test-notfalse.ps1 -logger $customLogger
...
& {
    ...
    $test.Data.err = try {$test.Data.fullHelp = Get-Help Test-NotFalse -Full} catch {$_}

    $test.Pass =
        ($null -eq $test.Data.err) -and
        ($test.Data.fullHelp.Name -is [System.String]) -and
        ($test.Data.fullHelp.Name.Equals('Test-NotFalse', [System.StringComparison]::OrdinalIgnoreCase)) -and
...
& {
    ...
    $message = 'Test-NotFalse failed parameter tests.'

    $test.Data.paramSets = @((Get-Command -Name Test-NotFalse).ParameterSets)
...
& {
    ...
    $test.Data.err = try {Test-NotFalse $true -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    ...
    $test.Data.err = try {Test-NotFalse $false -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    ...
    $test.Data.err = try {Test-NotFalse $null -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Test Test-NotFalse with Non-Booleans that are convertible to $true'
    ...
    for ($i = 0; $i -lt $nonBooleanTrue.Count; $i++) {
        $test.Data.err[$i] = try {Test-NotFalse $nonBooleanTrue[$i] -OutVariable out | Out-Null} catch {$_}
        $test.Data.out[$i] = $out
...
& {
    $test = newTestLogEntry 'Test Test-NotFalse with Non-Booleans that are convertible to $false'
    ...
    for ($i = 0; $i -lt $nonBooleanFalse.Count; $i++) {
        $test.Data.err[$i] = try {Test-NotFalse $nonBooleanFalse[$i] -OutVariable out | Out-Null} catch {$_}
        $test.Data.out[$i] = $out
