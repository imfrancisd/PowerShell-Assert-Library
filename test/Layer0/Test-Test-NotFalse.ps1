...
.\Test-Test-NotFalse.ps1 -logger $simpleLogger
...
.\Test-Test-NotFalse.ps1 -logger $customLogger
...
& {
    $test = newTestLogEntry 'Test Test-NotFalse with get-help -full'

    $test.Data.in  = @{name = 'Test-NotFalse'}
    ...
    $test.Pass =
        ...
        ($test.Data.out[0].Name.Equals('Test-NotFalse', [System.StringComparison]::OrdinalIgnoreCase)) -and
    ...
    if (-not $test.Pass) {
        $message = 'Test-NotFalse failed with get-help -full.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}

& {
    $test = newTestLogEntry 'Test Test-NotFalse parameters'
    ...
    if ($null -ne $test.Data.err) {
        $message = 'Test-NotFalse failed parameter tests (could not get command).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }

    $paramSets = @($test.Data.out[0].ParameterSets)
    if (1 -ne $paramSets.Count) {
        $message = 'Test-NotFalse failed parameter tests (more than one parameter set).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
    ...
    if ($null -eq $valueParam) {
        $message = 'Test-NotFalse failed parameter tests (no parameter with name "Value").'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
    ...
    if (-not $test.Pass) {
        $message = 'Test-NotFalse failed parameter tests ("Value" parameter attributes).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}

& {
    $test = newTestLogEntry 'Test Test-NotFalse with Boolean $true'

    $test.Data.in  = @{value = $true}
    $test.Data.err = try {Test-NotFalse $test.Data.in.value -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
    ...
    if (-not $test.Pass) {
        $message = 'Test-NotFalse failed with $true value.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}

& {
    $test = newTestLogEntry 'Test Test-NotFalse with Boolean $false'
    ...
    if (-not $test.Pass) {
        $message = 'Test-NotFalse failed with $false value.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}

& {
    $test = newTestLogEntry 'Test Test-NotFalse with $null'
    ...
    if (-not $test.Pass) {
        $message = 'Test-NotFalse failed with $null value.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
...
& {
    for ($i = 0; $i -lt $nonBooleanTrue.Count; $i++) {
        $test = newTestLogEntry 'Test Test-NotFalse with Non-Boolean that is convertible to $true'
        ...
        if (-not $test.Pass) {
            $message = 'Test-NotFalse failed with Non-Boolean that is convertible to $true.'
            throw New-Object 'System.Exception' -ArgumentList @($message)
        }
    }
    ...
    if ($i -eq 0) {
        ...
        $message = 'No data to test Test-NotFalse with Non-Boolean that is convertible to $true.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
...
& {
    for ($i = 0; $i -lt $nonBooleanFalse.Count; $i++) {
        $test = newTestLogEntry 'Test Test-NotFalse with Non-Boolean that is convertible to $false'
        ...
        if (-not $test.Pass) {
            $message = 'Test-NotFalse failed with Non-Boolean that is convertible to $false.'
            throw New-Object 'System.Exception' -ArgumentList @($message)
        }
    }
    ...
    if ($i -eq 0) {
        ...
        $message = 'No data to test Test-NotFalse with Non-Boolean that is convertible to $false.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
