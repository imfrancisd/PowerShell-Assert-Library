...
.\Test-Test-Null.ps1 -logger $simpleLogger
...
.\Test-Test-Null.ps1 -logger $customLogger
...
& {
    $test = newTestLogEntry 'Test Test-Null with get-help -full'

    $test.Data.in  = @{name = 'Test-Null'}
    ...
    $test.Pass =
        ...
        ($test.Data.out[0].Name.Equals('Test-Null', [System.StringComparison]::OrdinalIgnoreCase)) -and
...
& {
    $test = newTestLogEntry 'Test Test-Null parameters'
    ...
    if ($null -ne $test.Data.err) {
        $message = 'Test-Null failed parameter tests (could not get command).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }

    $paramSets = @($test.Data.out[0].ParameterSets)
    if (1 -ne $paramSets.Count) {
        $message = 'Test-Null failed parameter tests (more than one parameter set).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
    ...
    if ($null -eq $valueParam) {
        $message = 'Test-Null failed parameter tests (no parameter with name "Value").'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
    ...
    if (-not $test.Pass) {
        $message = 'Test-Null failed parameter tests ("Value" parameter attributes).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}

& {
    $test = newTestLogEntry 'Test Test-Null with Boolean $true'

    $test.Data.in  = @{value = $true}
    $test.Data.err = try {Test-Null $test.Data.in.value -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Test Test-Null with Boolean $false'
...
& {
    $test = newTestLogEntry 'Test Test-Null with $null'
...
& {
    for ($i = 0; $i -lt $nonBooleanTrue.Count; $i++) {
        $test = newTestLogEntry 'Test Test-Null with Non-Boolean that is convertible to $true'
        ...
        if (-not $test.Pass) {
            $message = 'Test-Null failed with Non-Boolean that is convertible to $true.'
            throw New-Object 'System.Exception' -ArgumentList @($message)
        }
    }
    ...
    if ($i -eq 0) {
        ...
        $message = 'No data to test Test-Null with Non-Boolean that is convertible to $true.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
...
& {
    for ($i = 0; $i -lt $nonBooleanFalse.Count; $i++) {
        $test = newTestLogEntry 'Test Test-Null with Non-Boolean that is convertible to $false'
        ...
        if (-not $test.Pass) {
            $message = 'Test-Null failed with Non-Boolean that is convertible to $false.'
            throw New-Object 'System.Exception' -ArgumentList @($message)
        }
    }
    ...
    if ($i -eq 0) {
        ...
        $message = 'No data to test Test-Null with Non-Boolean that is convertible to $false.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
