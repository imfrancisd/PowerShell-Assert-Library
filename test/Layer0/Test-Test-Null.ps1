...
.\test-test-null.ps1 -logger $simpleLogger
...
.\test-test-null.ps1 -logger $customLogger
...
& {
    ...
    $test.Data.err = try {$test.Data.fullHelp = Get-Help Test-Null -Full} catch {$_}

    $test.Pass =
        ($null -eq $test.Data.err) -and
        ($test.Data.fullHelp.Name -is [System.String]) -and
        ($test.Data.fullHelp.Name.Equals('Test-Null', [System.StringComparison]::OrdinalIgnoreCase)) -and
...
& {
    ...
    $test.Data.paramSets = @((Get-Command -Name Test-Null).ParameterSets)
...
& {
    ...
    $test.Data.err = try {Test-Null $true -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    ...
    $test.Data.err = try {Test-Null $false -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    ...
    $test.Data.err = try {Test-Null $null -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Test Test-Null with Non-Booleans that are convertible to $true'
    ...
    for ($i = 0; $i -lt $nonBooleanTrue.Count; $i++) {
        $test.Data.err[$i] = try {Test-Null $nonBooleanTrue[$i] -OutVariable out | Out-Null} catch {$_}
        $test.Data.out[$i] = $out
...
& {
    $test = newTestLogEntry 'Test Test-Null with Non-Booleans that are convertible to $false'
    ...
    for ($i = 0; $i -lt $nonBooleanFalse.Count; $i++) {
        $test.Data.err[$i] = try {Test-Null $nonBooleanFalse[$i] -OutVariable out | Out-Null} catch {$_}
        $test.Data.out[$i] = $out
