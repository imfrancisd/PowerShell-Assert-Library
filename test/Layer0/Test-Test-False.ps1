...
.\Test-Test-False.ps1 -logger $simpleLogger
...
.\Test-Test-False.ps1 -logger $customLogger
...
& {
    ...
    $test.Pass =
        ...
        ($test.Data.out[0].Name.Equals('Test-False', [System.StringComparison]::OrdinalIgnoreCase)) -and
...
& {
    ...
    if ($null -ne $test.Data.err) {
        $message = 'Test-False failed parameter tests (could not get command).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }

    $paramSets = @($test.Data.out[0].ParameterSets)
    if (1 -ne $paramSets.Count) {
        $message = 'Test-False failed parameter tests (more than one parameter set).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
    ...
    if ($null -eq $valueParam) {
        $message = 'Test-False failed parameter tests (no parameter with name "Value").'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
    ...
    if (-not $test.Pass) {
        $message = 'Test-False failed parameter tests ("Value" parameter attributes).'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
...
& {
    ...
    $test.Data.err = try {Test-False $test.Data.in.value -OutVariable out | Out-Null} catch {$_}
    $test.Data.out = $out
...
& {
    for ($i = 0; $i -lt $nonBooleanTrue.Count; $i++) {
        $test = newTestLogEntry 'Test Test-False with Non-Boolean that is convertible to $true'
    ...
    if ($i -eq 0) {
        ...
        $message = 'No data to test Test-False with Non-Boolean that is convertible to $true.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
...
& {
    for ($i = 0; $i -lt $nonBooleanFalse.Count; $i++) {
        $test = newTestLogEntry 'Test Test-False with Non-Boolean that is convertible to $false'
    ...
    if ($i -eq 0) {
        ...
        $message = 'No data to test Test-False with Non-Boolean that is convertible to $false.'
        throw New-Object 'System.Exception' -ArgumentList @($message)
    }
}
