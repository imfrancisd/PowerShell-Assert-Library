...
<#
.Synopsis
Test the Assert-PipelineAny cmdlet.
.Description
Test the Assert-PipelineAny cmdlet.
.Inputs
None
...
.\Test-Assert-PipelineAny.ps1 -logger $simpleLogger
...
.\Test-Assert-PipelineAny.ps1 -logger $customLogger
...
& {
    $test = newTestLogEntry 'Assert-PipelineAny help'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{name = 'Assert-PipelineAny'}
        ...
        Assert-True ($test.Data.out[0].Name.Equals('Assert-PipelineAny', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $test = newTestLogEntry 'Assert-PipelineAny parameters'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{name = 'Assert-PipelineAny'}
...
& {
    $test = newTestLogEntry 'Assert-PipelineAny with non-pipeline input'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{inputObject = @(1..3);}
        $test.Data.err = try {Assert-PipelineAny -InputObject $test.Data.in.inputObject -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out

        Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
        Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('PipelineArgumentOnly,Assert-PipelineAny', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $test = newTestLogEntry 'Assert-PipelineAny with pipeline containing nothing'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{inputObject = @();}
        $test.Data.err = try {$test.Data.in.inputObject | Assert-PipelineAny -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out

        Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
        Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('AssertionFailed,Assert-PipelineAny', [System.StringComparison]::OrdinalIgnoreCase))
        Assert-True ($test.Data.out.Count -eq 0)
...
& {
    $test = newTestLogEntry 'Assert-PipelineAny with pipeline containing $true'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineAny with pipeline containing $false'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineAny with pipeline containing $null'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $testDescription = 'Assert-PipelineAny with pipeline containing Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-PipelineAny with pipeline containing Non-Boolean that is convertible to $false'
...
& {
    $testDescription = 'Assert-PipelineAny with pipeline containing multiple items'
