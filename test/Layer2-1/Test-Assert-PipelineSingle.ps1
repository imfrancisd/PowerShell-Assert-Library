...
<#
.Synopsis
Test the Assert-PipelineSingle cmdlet.
.Description
Test the Assert-PipelineSingle cmdlet.
.Inputs
None
...
.\Test-Assert-PipelineSingle.ps1 -logger $simpleLogger
...
.\Test-Assert-PipelineSingle.ps1 -logger $customLogger
...
& {
    $test = newTestLogEntry 'Assert-PipelineSingle help'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{name = 'Assert-PipelineSingle'}
        ...
        Assert-True ($test.Data.out[0].Name.Equals('Assert-PipelineSingle', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $test = newTestLogEntry 'Assert-PipelineSingle parameters'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{name = 'Assert-PipelineSingle'}
...
& {
    $test = newTestLogEntry 'Assert-PipelineSingle with non-pipeline input'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{inputObject = @(1..3);}
        $test.Data.err = try {Assert-PipelineSingle -InputObject $test.Data.in.inputObject -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out

        Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
        Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('PipelineArgumentOnly,Assert-PipelineSingle', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $test = newTestLogEntry 'Assert-PipelineSingle with pipeline containing nothing'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{inputObject = @();}
        $test.Data.err = try {$test.Data.in.inputObject | Assert-PipelineSingle -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out

        Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
        Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('AssertionFailed,Assert-PipelineSingle', [System.StringComparison]::OrdinalIgnoreCase))
        Assert-True ($test.Data.out.Count -eq 0)
...
& {
    $test = newTestLogEntry 'Assert-PipelineSingle with pipeline containing $true'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineSingle with pipeline containing $false'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineSingle with pipeline containing $null'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $testDescription = 'Assert-PipelineSingle with pipeline containing Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-PipelineSingle with pipeline containing Non-Boolean that is convertible to $false'
...
& {
    $testDescription = 'Assert-PipelineSingle with pipeline containing multiple items'
    ...
    for ($i = 0; $i -lt $items.Count; $i++) {
        ...
        try {
            ...
            if ($test.Data.in.inputObject.Count -eq 1) {
                Assert-Null ($test.Data.err)
            } else {
                Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
                Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('AssertionFailed,Assert-PipelineSingle', [System.StringComparison]::OrdinalIgnoreCase))
