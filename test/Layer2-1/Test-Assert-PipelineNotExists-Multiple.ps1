...
.\Test-Assert-PipelineNotExists-Multiple.ps1 -logger $simpleLogger
...
.\Test-Assert-PipelineNotExists-Multiple.ps1 -logger $customLogger
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with non-pipeline input'
    ...
    for ($i = 0; $i -lt $predicates.Count; $i++) {
        ...
        try {
            ...
            $test.Data.err = try {Assert-PipelineNotExists -Quantity Multiple -InputObject $test.Data.in.inputObject $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with null predicate'
    ...
    for ($i = 0; $i -lt $collections.Count; $i++) {
        ...
        try {
            ...
            $test.Data.err = try {$test.Data.in.inputObject | Assert-PipelineNotExists -Quantity Multiple $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing nothing'
    $predicates = @($predicates.psbase.Values)
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing one item and true predicate'
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing two items and true predicate'
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing one item and false predicate'
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing two items and false predicate'
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing one item and identity predicate'
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing one item and error predicate'
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing one Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing two Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing one Non-Boolean that is convertible to $false'
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing two Non-Boolean that is convertible to $false'
...
& {
    $testDescription = 'Assert-PipelineNotExists -Quantity Multiple with pipeline containing multiple items'
...
& {
    $test = newTestLogEntry 'Assert-PipelineNotExists -Quantity Multiple with a predicate that throws on the first element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineNotExists -Quantity Multiple with a predicate that throws on the second element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineNotExists -Quantity Multiple with a predicate that throws on the third element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineNotExists -Quantity Multiple with a predicate that throws on the fourth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineNotExists -Quantity Multiple with a predicate that throws on the fifth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    ...
    foreach ($i in @(0..($numbers.Length + 1))) {
        $test = newTestLogEntry 'Assert-PipelineNotExists -Quantity Multiple normal pass'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
...
& {
    ...
    foreach ($i in @(1..4)) {
        $test = newTestLogEntry 'Assert-PipelineNotExists -Quantity Multiple early fail'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
            ...
            Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
            Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('AssertionFailed,Assert-PipelineNotExists', [System.StringComparison]::OrdinalIgnoreCase))
            Assert-True ($test.Data.out.Count -eq $i + 4 - 1)
            Assert-All @(0..($i + 4 - 1 - 1)) {param($index) $test.Data.in.inputObject[$index].Equals($test.Data.out[$index])}
            Assert-True (0 -eq $test.Data.in.remainingCalls)
...
& {
    ...
    $test = newTestLogEntry 'Assert-PipelineNotExists -Quantity Multiple with a predicate that contains "break" outside of a loop'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in = @{
