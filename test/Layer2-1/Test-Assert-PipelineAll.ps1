...
<#
.Synopsis
Test the Assert-PipelineAll cmdlet.
.Description
Test the Assert-PipelineAll cmdlet.
.Inputs
None
...
.\Test-Assert-PipelineAll.ps1 -logger $simpleLogger
...
.\Test-Assert-PipelineAll.ps1 -logger $customLogger
...
& {
    $test = newTestLogEntry 'Assert-PipelineAll help'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{name = 'Assert-PipelineAll'}
        ...
        Assert-True ($test.Data.out[0].Name.Equals('Assert-PipelineAll', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $test = newTestLogEntry 'Assert-PipelineAll parameters'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{name = 'Assert-PipelineAll'}
...
& {
    $testDescription = 'Assert-PipelineAll with non-pipeline input'
    ...
    for ($i = 0; $i -lt $predicates.Count; $i++) {
        ...
        try {
            ...
            $test.Data.err = try {Assert-PipelineAll -InputObject $test.Data.in.inputObject $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out

            Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
            Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('PipelineArgumentOnly,Assert-PipelineAll', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $testDescription = 'Assert-PipelineAll with null predicate'
    ...
    for ($i = 0; $i -lt $collections.Count; $i++) {
        ...
        try {
            ...
            $test.Data.err = try {$test.Data.in.inputObject | Assert-PipelineAll $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out

            Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
            Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('ParameterArgumentValidationErrorNullNotAllowed,Assert-PipelineAll', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $testDescription = 'Assert-PipelineAll with pipeline containing nothing'
    $predicates = @($predicates.psbase.Values)
...
& {
    $testDescription = 'Assert-PipelineAll with pipeline containing scalar and true predicate'
...
& {
    $testDescription = 'Assert-PipelineAll with pipeline containing scalar and false predicate'
    ...
    for ($i = 0; $i -lt $nonCollections.Count; $i++) {
        ...
        try {
            ...
            Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
            Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('AssertionFailed,Assert-PipelineAll', [System.StringComparison]::OrdinalIgnoreCase))
            Assert-True ($test.Data.out.Count -eq 0)
...
& {
    $testDescription = 'Assert-PipelineAll with pipeline containing scalar and identity predicate'
...
& {
    $testDescription = 'Assert-PipelineAll with pipeline containing scalar and error predicate'
    ...
    for ($i = 0; $i -lt $nonCollections.Count; $i++) {
        ...
        try {
            ...
            Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
            Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('PredicateFailed,Assert-PipelineAll', [System.StringComparison]::OrdinalIgnoreCase))
            Assert-True ($test.Data.out.Count -eq 0)
...
& {
    $testDescription = 'Assert-PipelineAll with pipeline containing Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-PipelineAll with pipeline containing Non-Boolean that is convertible to $false'
...
& {
    $testDescription = 'Assert-PipelineAll with pipeline containing multiple items'
    ...
    for ($i = 0; $i -lt $items.Count; $i++) {
        ...
        try {
            $test.Data.out = $out = @()
            $test.Data.in  = @{inputObject = @($items[0..$i]); predicate = $predicates.True;}
            $test.Data.err = try {$test.Data.in.inputObject | Assert-PipelineAll $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Assert-PipelineAll with a predicate that throws on the first element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{inputObject = @(1, 2, 3, 4); predicate = {param($a) if ($a -eq 1) {throw "Bad predicate $a"} else {$true}};}
        $test.Data.err = try {$test.Data.in.inputObject | Assert-PipelineAll $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Assert-PipelineAll with a predicate that throws on the second element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{inputObject = @(1, 2, 3, 4); predicate = {param($a) if ($a -eq 2) {throw "Bad predicate $a"} else {$true}};}
        $test.Data.err = try {$test.Data.in.inputObject | Assert-PipelineAll $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Assert-PipelineAll with a predicate that throws on the third element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{inputObject = @(1, 2, 3, 4); predicate = {param($a) if ($a -eq 3) {throw "Bad predicate $a"} else {$true}};}
        $test.Data.err = try {$test.Data.in.inputObject | Assert-PipelineAll $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Assert-PipelineAll with a predicate that throws on the fourth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{inputObject = @(1, 2, 3, 4); predicate = {param($a) if ($a -eq 4) {throw "Bad predicate $a"} else {$true}};}
        $test.Data.err = try {$test.Data.in.inputObject | Assert-PipelineAll $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Assert-PipelineAll with a predicate that throws on the fifth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{inputObject = @(1, 2, 3, 4); predicate = {param($a) if ($a -eq 5) {throw "Bad predicate $a"} else {$true}};}
        $test.Data.err = try {$test.Data.in.inputObject | Assert-PipelineAll $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    ...
    foreach ($i in @(1..5)) {
        $test = newTestLogEntry 'Assert-PipelineAll early fail'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
            ...
            Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
            Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('AssertionFailed,Assert-PipelineAll', [System.StringComparison]::OrdinalIgnoreCase))
            Assert-True ($test.Data.out.Count -eq ($i - 1))
            if ($test.Data.out.Count -gt 0) {
                Assert-All @(0..$($test.Data.out.Count - 1)) {param($index) $test.Data.in.inputObject[$index].Equals($test.Data.out[$index])}
            }
...
& {
    ...
    $test = newTestLogEntry 'Assert-PipelineAll with a predicate that contains "break" outside of a loop'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in = @{
