...
.\Test-Assert-PipelineExists-Any.ps1 -logger $simpleLogger
...
.\Test-Assert-PipelineExists-Any.ps1 -logger $customLogger
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with non-pipeline input'
    ...
    for ($i = 0; $i -lt $predicates.Count; $i++) {
        ...
        try {
            ...
            $test.Data.err = try {Assert-PipelineExists -Quantity Any -InputObject $test.Data.in.inputObject $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with null predicate'
    ...
    for ($i = 0; $i -lt $collections.Count; $i++) {
        ...
        try {
            ...
            $test.Data.err = try {$test.Data.in.inputObject | Assert-PipelineExists -Quantity Any $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing nothing'
    $predicates = @($predicates.psbase.Values)
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing one item and true predicate'
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing two items and true predicate'
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing one item and false predicate'
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing two items and false predicate'
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing one item and identity predicate'
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing one item and error predicate'
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing one Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing two Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing one Non-Boolean that is convertible to $false'
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing two Non-Boolean that is convertible to $false'
...
& {
    $testDescription = 'Assert-PipelineExists -Quantity Any with pipeline containing multiple items'
...
& {
    $test = newTestLogEntry 'Assert-PipelineExists -Quantity Any with a predicate that throws on the first element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineExists -Quantity Any with a predicate that throws on the second element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineExists -Quantity Any with a predicate that throws on the third element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineExists -Quantity Any with a predicate that throws on the fourth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-PipelineExists -Quantity Any with a predicate that throws on the fifth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    ...
    foreach ($i in @(0, 6, -1)) {
        $test = newTestLogEntry 'Assert-PipelineExists -Quantity Any normal fail'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
...
& {
    ...
    foreach ($i in @(1..4)) {
        $test = newTestLogEntry 'Assert-PipelineExists -Quantity Any early pass'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
...
& {
    ...
    $test = newTestLogEntry 'Assert-PipelineExists -Quantity Any with a predicate that contains "break" outside of a loop'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in = @{
