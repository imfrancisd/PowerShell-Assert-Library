...
.\Test-Assert-Exists.ps1 -logger $simpleLogger
...
.\Test-Assert-Exists.ps1 -logger $customLogger
...
& {
    $test = newTestLogEntry 'Assert-Exists help'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{name = 'Assert-Exists'}
        ...
        Assert-True ($test.Data.out[0].Name.Equals('Assert-Exists', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $test = newTestLogEntry 'Assert-Exists parameters'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{name = 'Assert-Exists'}
...
& {
    $test = newTestLogEntry 'Assert-Exists with singleton containing $true'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{collection = @($true); predicate = $predicates.Identity;}
        $test.Data.err = try {Assert-Exists $test.Data.in.collection $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Assert-Exists with singleton containing $false'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists with singleton containing $null'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $testDescription = 'Assert-Exists with singleton containing Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-Exists with singleton containing Non-Boolean that is convertible to $false'
...
& {
    $test = newTestLogEntry 'Assert-Exists with tuple containing $true'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists with tuple containing $false'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists with tuple containing $null'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $testDescription = 'Assert-Exists with tuple containing Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-Exists with tuple containing Non-Boolean that is convertible to $false'
...
& {
    $testDescription = 'Assert-Exists with null predicate'
...
& {
    $testDescription = 'Assert-Exists with null collection'
...
& {
    $testDescription = 'Assert-Exists with collections containing null'
...
& {
    $testDescription = 'Assert-Exists with empty collection and true predicate'
...
& {
    $testDescription = 'Assert-Exists with empty collection and false predicate'
...
& {
    $testDescription = 'Assert-Exists with empty collection and identity predicate'
...
& {
    $testDescription = 'Assert-Exists with empty collection and error predicate'
...
& {
    $testDescription = 'Assert-Exists with scalar and true predicate'
...
& {
    $testDescription = 'Assert-Exists with scalar and false predicate'
...
& {
    $testDescription = 'Assert-Exists with scalar and identity predicate'
...
& {
    $testDescription = 'Assert-Exists with scalar and error predicate'
...
& {
    $test = newTestLogEntry 'Assert-Exists with quadruple and a predicate that throws on the first element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists with quadruple and a predicate that throws on the second element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists with quadruple and a predicate that throws on the third element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists with quadruple and a predicate that throws on the fourth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists with quadruple and a predicate that throws on the fifth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    ...
    foreach ($i in @(0, 6, -1)) {
        $test = newTestLogEntry 'Assert-Exists normal fail'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
            ...
            $test.Data.err = try {Assert-Exists $test.Data.in.inputObject $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out
...
& {
    ...
    foreach ($i in @(1..4)) {
        $test = newTestLogEntry 'Assert-Exists early pass'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
...
& {
    ...
    $test = newTestLogEntry 'Assert-Exists with a predicate that contains "break" outside of a loop'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in = @{
...
& {
    $test = newTestLogEntry 'Assert-Exists with an arraylist that has its .NET members overriden'
    $pass = $false
...
& {
    $test = newTestLogEntry 'Assert-Exists with a hashtable that has its .NET members overriden'
    $pass = $false
