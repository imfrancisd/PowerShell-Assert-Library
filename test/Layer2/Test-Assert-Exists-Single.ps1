...
.\Test-Assert-Exists-Single.ps1 -logger $simpleLogger
...
.\Test-Assert-Exists-Single.ps1 -logger $customLogger
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with singleton containing $true'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{collection = @($true); predicate = $predicates.Identity;}
        $test.Data.err = try {Assert-Exists -Quantity Single $test.Data.in.collection $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with singleton containing $false'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with singleton containing $null'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with singleton containing Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with singleton containing Non-Boolean that is convertible to $false'
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with tuple containing $true'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with tuple containing $false'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with tuple containing $null'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with tuple containing Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with tuple containing Non-Boolean that is convertible to $false'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with null predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with null collection'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with collections containing null'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with empty collection and true predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with empty collection and false predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with empty collection and identity predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with empty collection and error predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with scalar and true predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with scalar and false predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with scalar and identity predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Single with scalar and error predicate'
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with quadruple and a predicate that throws on the first element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with quadruple and a predicate that throws on the second element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with quadruple and a predicate that throws on the third element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with quadruple and a predicate that throws on the fourth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with quadruple and a predicate that throws on the fifth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    ...
    foreach ($i in @(1..$numbers.Length)) {
        $test = newTestLogEntry 'Assert-Exists -Quantity Single normal pass'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
            ...
            $test.Data.err = try {Assert-Exists $test.Data.in.inputObject -Quantity Single $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out
...
& {
    ...
    foreach ($i in @(1..4)) {
        $test = newTestLogEntry 'Assert-Exists -Quantity Single early fail'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
...
& {
    ...
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with a predicate that contains "break" outside of a loop'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in = @{
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with an arraylist that has its .NET members overriden'
    $pass = $false
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Single with a hashtable that has its .NET members overriden'
    $pass = $false
