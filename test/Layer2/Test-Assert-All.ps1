...
.\Test-Assert-All.ps1 -logger $simpleLogger
...
.\Test-Assert-All.ps1 -logger $customLogger
...
& {
    $test = newTestLogEntry 'Assert-All help'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{name = 'Assert-All'}
        ...
        Assert-True ($test.Data.out[0].Name.Equals('Assert-All', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $test = newTestLogEntry 'Assert-All parameters'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{name = 'Assert-All'}
...
& {
    $test = newTestLogEntry 'Assert-All with singleton containing $true'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{collection = @($true); predicate = $predicates.Identity;}
        $test.Data.err = try {Assert-All $test.Data.in.collection $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Assert-All with singleton containing $false'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        ...
        Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('AssertionFailed,Assert-All', [System.StringComparison]::OrdinalIgnoreCase))
        Assert-True ($test.Data.out.Count -eq 0)
...
& {
    $test = newTestLogEntry 'Assert-All with singleton containing $null'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $testDescription = 'Assert-All with singleton containing Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-All with singleton containing Non-Boolean that is convertible to $false'
...
& {
    $testDescription = 'Assert-All with null predicate'
    ...
    for ($i = 0; $i -lt $collections.Count; $i++) {
        ...
        try {
            ...
            Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
            Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('ParameterArgumentValidationErrorNullNotAllowed,Assert-All', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $testDescription = 'Assert-All with null collection'
...
& {
    $testDescription = 'Assert-All with collections containing null'
    ...
    for ($i = 0; $i -lt $collections.Count; $i++) {
        ...
        try {
            $test.Data.out = $out = @()
            $test.Data.in  = @{collection = $collections[$i]; predicate = $predicates.True;}
            $test.Data.err = try {Assert-All $test.Data.in.collection $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out
...
& {
    $testDescription = 'Assert-All with empty collection and true predicate'
...
& {
    $testDescription = 'Assert-All with empty collection and false predicate'
...
& {
    $testDescription = 'Assert-All with empty collection and identity predicate'
...
& {
    $testDescription = 'Assert-All with empty collection and error predicate'
...
& {
    $testDescription = 'Assert-All with scalar and true predicate'
...
& {
    $testDescription = 'Assert-All with scalar and false predicate'
...
& {
    $testDescription = 'Assert-All with scalar and identity predicate'
...
& {
    $testDescription = 'Assert-All with scalar and error predicate'
...
& {
    $test = newTestLogEntry 'Assert-All with quadruple and a predicate that throws on the first element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{collection = @(1, 2, 3, 4); predicate = {param($a) if ($a -eq 1) {throw "Bad predicate $a"} else {$true}};}
        $test.Data.err = try {Assert-All $test.Data.in.collection $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out

        Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
        Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('PredicateFailed,Assert-All', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $test = newTestLogEntry 'Assert-All with quadruple and a predicate that throws on the second element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{collection = @(1, 2, 3, 4); predicate = {param($a) if ($a -eq 2) {throw "Bad predicate $a"} else {$true}};}
        $test.Data.err = try {Assert-All $test.Data.in.collection $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Assert-All with quadruple and a predicate that throws on the third element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{collection = @(1, 2, 3, 4); predicate = {param($a) if ($a -eq 3) {throw "Bad predicate $a"} else {$true}};}
        $test.Data.err = try {Assert-All $test.Data.in.collection $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Assert-All with quadruple and a predicate that throws on the fourth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{collection = @(1, 2, 3, 4); predicate = {param($a) if ($a -eq 4) {throw "Bad predicate $a"} else {$true}};}
        $test.Data.err = try {Assert-All $test.Data.in.collection $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Assert-All with quadruple and a predicate that throws on the fifth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{collection = @(1, 2, 3, 4); predicate = {param($a) if ($a -eq 5) {throw "Bad predicate $a"} else {$true}};}
        $test.Data.err = try {Assert-All $test.Data.in.collection $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out
...
& {
    ...
    foreach ($i in @(1..5)) {
        $test = newTestLogEntry 'Assert-All early fail'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
...
& {
    ...
    $test = newTestLogEntry 'Assert-All with a predicate that contains "break" outside of a loop'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in = @{
...
& {
    $test = newTestLogEntry 'Assert-All with an arraylist that has its .NET members overriden'
    $pass = $false
...
& {
    $test = newTestLogEntry 'Assert-All with a hashtable that has its .NET members overriden'
    $pass = $false
