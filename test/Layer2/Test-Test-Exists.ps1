...
& {
    $test = newTestLogEntry 'Test-Exists with tuple containing $true'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Test-Exists with tuple containing $false'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Test-Exists with tuple containing $null'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $testDescription = 'Test-Exists with tuple containing Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Test-Exists with tuple containing Non-Boolean that is convertible to $false'
...
& {
    ...
    foreach ($i in @(0, 6, -1)) {
        $test = newTestLogEntry 'Test-Exists normal fail'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
            ...
            $test.Data.err = try {Test-Exists $test.Data.in.inputObject $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out
...
& {
    $test = newTestLogEntry 'Test-Exists with an arraylist that has its .NET members overriden'
    $pass = $false
...
& {
    $test = newTestLogEntry 'Test-Exists with a hashtable that has its .NET members overriden'
    $pass = $false
