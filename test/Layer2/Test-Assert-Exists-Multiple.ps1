...
.\Test-Assert-Exists-Multiple.ps1 -logger $simpleLogger
...
.\Test-Assert-Exists-Multiple.ps1 -logger $customLogger
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with singleton containing $true'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in  = @{collection = @($true); predicate = $predicates.Identity;}
        $test.Data.err = try {Assert-Exists -Quantity Multiple $test.Data.in.collection $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
        $test.Data.out = $out

        Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
        Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('AssertionFailed,Assert-Exists', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with singleton containing $false'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with singleton containing $null'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with singleton containing Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with singleton containing Non-Boolean that is convertible to $false'
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with tuple containing $true'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with tuple containing $false'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with tuple containing $null'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with tuple containing Non-Boolean that is convertible to $true'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with tuple containing Non-Boolean that is convertible to $false'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with null predicate'
    ...
    for ($i = 0; $i -lt $collections.Count; $i++) {
        ...
        try {
            ...
            Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
            Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('ParameterArgumentValidationErrorNullNotAllowed,Assert-Exists', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with null collection'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with collections containing null'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with empty collection and true predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with empty collection and false predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with empty collection and identity predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with empty collection and error predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with scalar and true predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with scalar and false predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with scalar and identity predicate'
...
& {
    $testDescription = 'Assert-Exists -Quantity Multiple with scalar and error predicate'
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with quadruple and a predicate that throws on the first element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        ...
        Assert-True ($test.Data.err -is [System.Management.Automation.ErrorRecord])
        Assert-True ($test.Data.err.FullyQualifiedErrorId.Equals('PredicateFailed,Assert-Exists', [System.StringComparison]::OrdinalIgnoreCase))
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with quadruple and a predicate that throws on the second element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with quadruple and a predicate that throws on the third element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with quadruple and a predicate that throws on the fourth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with quadruple and a predicate that throws on the fifth element'
    $pass = $false
    try {
        $test.Data.out = $out = @()
...
& {
    ...
    foreach ($i in @(0, 6, -1)) {
        $test = newTestLogEntry 'Assert-Exists -Quantity Multiple normal fail'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
            ...
            $test.Data.err = try {Assert-Exists -Quantity Multiple $test.Data.in.inputObject $test.Data.in.predicate -OutVariable out | Out-Null} catch {$_}
            $test.Data.out = $out
...
& {
    ...
    foreach ($i in @(1..4)) {
        $test = newTestLogEntry 'Assert-Exists -Quantity Multiple early pass'
        $pass = $false
        try {
            $test.Data.out = $out = @()
            $test.Data.in = @{
...
& {
    ...
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with a predicate that contains "break" outside of a loop'
    $pass = $false
    try {
        $test.Data.out = $out = @()
        $test.Data.in = @{
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with an arraylist that has its .NET members overriden'
    $pass = $false
...
& {
    $test = newTestLogEntry 'Assert-Exists -Quantity Multiple with a hashtable that has its .NET members overriden'
    $pass = $false
